...
# QB is deliberately excluded from the replacement-level path.
#
# Quarterback appearances are sequential and reconcile_qb_projected_volume_games
# allocates every room exactly 17 volume-games, so a QB row is not an
# additional candidate - it is a claim against the starter's exposure. Filling
# one from a band mean does real damage: Cleveland's curated QB2 is Deshaun
# Watson, whose absence from the models is itself the signal, and giving him
# the average curated-QB2 availability cut Shedeur Sanders from 13.6 games to
# 8.0. A missing charted QB is surfaced by the curation tripwire for a human
# to judge instead, which is the honest treatment of a player whose own team
# does not know if he will play.
#
# RB/WR/TE have no such fixed budget - an extra body genuinely adds a
# claimant to a volume pool - which is why the original diagnosis named
# exactly those three positions.
REPLACEMENT_POSITIONS = ("RB", "WR", "TE")
REPLACEMENT_MIN_CELL = 15
# Depth bands the replacement prior is fit over. Deliberately coarser than
# DEPTH_RATE_LADDER's rungs: this prior is only ever consulted for a player
# with no source-season production at all, so the sample it draws from must
# stay large enough to mean something.
