"""Availability-once composition contract for QB evaluation.
...
def detect_double_availability(
    ...
    *,
    ...
    twice_via_sched = (
        attempts_per_active * expected_active_starts * (expected_active_starts / SEASON_GAMES)
    )
    twice_via_17 = (
        (attempts_per_active * expected_active_starts / SEASON_GAMES) * expected_active_starts
    )
    return {
...
def allocate_starter_backup_season(
    *,
    ...
    missed = max(0.0, float(SEASON_GAMES) - starter_expected_starts)
