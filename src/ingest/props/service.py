...
def build_providers(
    *,
    ...
    ``mode=live`` uses real DraftKings + FanDuel (+ opt-in BettingPros) fetch for
    named live-capable books. Unknown names return a disabled stub
    (``live_provider_not_implemented``) so other books still succeed.
    ``mode=fixture`` loads local JSON fixtures.
    """
