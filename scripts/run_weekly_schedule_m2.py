"""Run Milestone 2 weekly team-week latent (research/shadow only).

Does not promote, reseal, train M3, or change League Value / freeze knobs / PWA.

Opponent/environment multipliers may move season team volume vs the sealed
prior. Same-week team_attempts / team_carries / team_targets / team_air_yards
are refused. ADP and season-long Vegas are not drivers.
...
from src.projection.weekly_latent.run import run_milestone2  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Milestone 2: team-week latent that may move season totals (shadow only)."
    )
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Shadow output directory (default: output/shadow_weekly_schedule_m2).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
